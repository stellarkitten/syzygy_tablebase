...
s = scatter(rtbw, rtbz, zcolor=len, c=:rainbow, colorbar=true, colorbar_title="Pieces", xscale=:log10, yscale=:log10, xlabel="RTBW/WDL File Size (bytes)", ylabel="RTBZ/DTZ File Size (bytes)", title="File Sizes", legend=false)
display(s)

h = histogram(longest, bins=maximum(longest), yscale=:log10, bar_width=1, linecolor=nothing, xlabel="DTZ Moves", ylabel="Endgames", title="Longest DTZ Histogram", legend=false)
display(h)

b = bar(range(0, step=1, length=length(dtz)), dtz, bar_width=1, linecolor=nothing, yscale=:log10, xlabel="DTZ Moves", ylabel="Positions", title="DTZ Bar Plot", legend=false)
display(b)

s = scatter(wins, losses, zcolor=draws, c=:rainbow, colorbar=true, colorbar_title="Draw Rate", xlabel="Win Rate", ylabel="Loss Rate",  title="WDL Distribution", legend=false)
display(s)
